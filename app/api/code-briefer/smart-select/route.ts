...
function stripDynamicSegments(p: string): string {
  return p
    .split("/")
...
Reply with only a JSON array of file paths. Example: ["app/tools/text-compare/page.tsx", "app/globals.css"]`;
  ...
  try {
    ...
    const mapped = selected.map((p) => {
      ...
      const stripped = stripDynamicSegments(p);
      const strippedLower = stripped.toLowerCase();
      const lower = p.toLowerCase();
      const exact = validPaths.find(
        (vp) =>
          vp.toLowerCase() === lower ||
          stripDynamicSegments(vp).toLowerCase() === strippedLower,
      );
...
function layerKeywords(prompt: string, paths: string[]): string[] {
  ...
  const dotTokens = raw.match(/[a-z0-9_-]+\.[a-z0-9_.-]+/g) ?? [];
  const regularWords = raw
    .replace(/\[|\]/g, " ")
    .replace(/[^a-z0-9\-_.]/g, " ")
    .split(/\s+/)
    .filter((w) => w.length > 1 && !w.includes(".") && !stopWords.has(w));

  const words = [...new Set([...dotTokens, ...regularWords])];
  if (words.length === 0) return [];

  const scored = paths.map((p) => {
    const strippedPath = stripDynamicSegments(p);
    const lower = strippedPath.toLowerCase();
    ...
    const score = words.reduce((acc, word) => {
      if (word.includes(".")) {
        ...
        const dotMatch =
          filename === word ||
          filename.startsWith(word.replace(/\.[^.]+$/, "."))
            ? 3
            : lower.includes(word)
              ? 1
              : 0;
